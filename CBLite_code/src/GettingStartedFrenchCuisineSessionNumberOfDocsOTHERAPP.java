...
public class GettingStartedFrenchCuisineSessionNumberOfDocsOTHERAPP {

	private static final int numberOfDocs = 10;
	private static boolean writeNewDocument = false;
	private static final String SET_COOKIE = "Set-Cookie";
	private static final String SG_COOKIE_NAME = "SyncGatewaySession";
	private static final String SGW_SESSION_URL = "http://sync-gateway:4984/french_cuisine/_session";
	 ...
	 * in production code
	 */
	private static final String DB_USER = "paul"; // paul (Bretagne) OR wolfgang (Alsace) marius (PACA)
	private static final String DB_PASS = "password";

	private static final String SYNC_GATEWAY_URL = "ws://sync-gateway:4984/french_cuisine"; // "ws://52.174.108.107:4984/stime";
	...
	public static void main(String[] args) throws CouchbaseLiteException, InterruptedException, URISyntaxException {
		...
		if (writeNewDocument) {
			...
			MutableDocument mutableDoc = new MutableDocument("produit_from_CBL_" + i)
...
//		System.out.println(String.format("Total rows returned by query = %d", numRows));
		...
		// replConfig.setAuthenticator(new SessionAuthenticator(tokenID,
		// SG_COOKIE_NAME));
...
//		}
		...
		// Live query
		Query queryAll2 = QueryBuilder.select(SelectResult.expression(Meta.id), SelectResult.property(Prop_Name),
				SelectResult.property(Prop_Price), SelectResult.property(Prop_Type),
				SelectResult.property(Prop_Channels)).from(DataSource.database(database));

		// Adds a query change listener.
		// Changes will be posted on the main queue.
		ListenerToken token = queryAll2.addChangeListener(change -> {
		    for (Result result2 : change.getResults()) {
		    	for(String key : result2.getKeys()) {
			    	System.out.println("Live query : change result key => " + key);
			    	 /* Update UI */
		    	}
		    }
		});
	...
	private static String getTokenID(String dbUser, String dbPass) {
		...
		// obtain the id_token.
		HttpResponse<String> response2 = Unirest.post(SGW_SESSION_URL).header("Content-type", "application/json")
				.body(new JSONObject("{'name':" + dbUser + ", 'password':" + dbPass + "}")).asString();
